...
\addtolength{\oddsidemargin}{-.26in}
\addtolength{\evensidemargin}{-.26in}
\addtolength{\textwidth}{0.52in}
...
%		\subsubsection{The PUT Method:}
	...
	\subsection{Round Trip Time Calculations}
		...
		The results obtained from these calculations were written to a text file and plotted using a graphing library, the results can be seen in Figure~\ref{fig:rtt} of Appendix~\ref{app:rtt}. The tests were conducted on retrieving the same \texttt{/index.html} for each situation. The results obtained are as expected, whereby the fastest time was TCP with a persistent connection and no proxy while the slowest time was a TCP non-persistent connection going through the proxy. \\
		
		The same experiment was conducted to determine the effectiveness of the cache on the rate at which the information is retrieved by the client. The results can be seen in Figure~\ref{fig:rtt_cache} of Appendix~\ref{app:rtt}, it can be noted that the once the \texttt{/index.html} file had been cached the client was able to retrieve the information faster.
...
\section{Code Description Supporting Information}
	...
	\label{app:proxy}
		...
		\lstinputlisting[caption=Conditional GET, breaklines=true]{../../README.md}
