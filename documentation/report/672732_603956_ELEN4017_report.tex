...
\section{DESCRIPTION OF CODE}
	...
	\subsection{Server Code}
		...
		\subsubsection*{Taking Action and Generating Response Message:} The HTTP method will determine the course of action that the server follows, these are detailed in Table~\ref{httpreqs}. The methods of interest are the GET and POST methods. The GET method firstly requires the server to check if the file exists and secondly, if it does exist, to retrieve a copy of the file and parse it into the response message to be sent to the client. If the file does not exist or there was another problem with the client request message, the server would generate the appropriate error message detailed in Table~\ref{httpresps}. The post method is of interest because after performing the same checks it either need to edit the found file or create the file on the server if the client has the correct permissions. 
